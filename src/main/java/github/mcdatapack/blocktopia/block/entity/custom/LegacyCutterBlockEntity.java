...
public class LegacyCutterBlockEntity extends LockableContainerBlockEntity implements ExtendedScreenHandlerFactory<BlockPosPayload>, SidedInventory {
    ...
    public LegacyCutterBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntityTypes.LEGACY_CUTTER, pos, state);
    ...
    private Optional<RecipeEntry<LegacyCutterRecipe>> getCurrentRecipe() {
        ...
        return getWorld().getRecipeManager().getFirstMatch(ModRecipes.LEGACY_CUTTER_RECIPE_TYPE, new SingleStackRecipeInput(getStack(INPUT_SLOT)), getWorld());
    }
