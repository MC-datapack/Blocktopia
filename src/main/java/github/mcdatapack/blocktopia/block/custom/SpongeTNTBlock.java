...
public class SpongeTNTBlock extends Block {
    public static final MapCodec<SpongeTNTBlock> CODEC = createCodec(SpongeTNTBlock::new);
    public static final BooleanProperty UNSTABLE = Properties.UNSTABLE;

    @Override
    public MapCodec<SpongeTNTBlock> getCodec() {
        return CODEC;
    }

    public SpongeTNTBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void onDestroyedByExplosion(World world, BlockPos pos, Explosion explosion) {
        if (!world.isClient) {
            SpongeTNTEntity tntEntity = new SpongeTNTEntity(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, explosion.getCausingEntity());
    ...
    private static void primeTnt(World world, BlockPos pos, @Nullable LivingEntity igniter) {
        if (!world.isClient) {
            SpongeTNTEntity tntEntity = new SpongeTNTEntity(world, pos.getX() + 0.5, pos.getY(), pos.getZ() + 0.5, igniter);
            world.spawnEntity(tntEntity);
