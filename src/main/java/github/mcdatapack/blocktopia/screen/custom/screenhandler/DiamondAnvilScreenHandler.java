...
public class DiamondAnvilScreenHandler extends ForgingScreenHandler {
    ...
    public DiamondAnvilScreenHandler(int syncId, PlayerInventory inventory, ScreenHandlerContext context) {
        super(ModScreenHandlerTypes.DIAMOND_ANVIL, syncId, inventory, context);
    ...
    protected void onTakeOutput(PlayerEntity player, ItemStack stack) {
        ...
        this.context.run((world, pos) -> {
            ...
            if (!player.isInCreativeMode() && blockState.isIn(BlockTags.ANVIL) && player.getRandom().nextFloat() < 0.25F) {
                BlockState blockState2 = DiamondAnvilBlock.getLandingState(blockState);
                if (blockState2 == null) {
    ...
    private boolean canBeCombined(RegistryEntry<Enchantment> first, RegistryEntry<Enchantment> second) {
        if ((first.getKey().get() == Enchantments.FORTUNE && second.getKey().get() == Enchantments.SILK_TOUCH) ||
            ...
            return false;
                ...
                (second.getKey().get() == Enchantments.FORTUNE && first.getKey().get() == Enchantments.DEPTH_STRIDER))
            return false;
        return true;
    }
