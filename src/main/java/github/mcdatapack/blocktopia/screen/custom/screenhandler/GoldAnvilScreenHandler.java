...
public class GoldAnvilScreenHandler extends ForgingScreenHandler {
    ...
    protected void onTakeOutput(PlayerEntity player, ItemStack stack) {
        ...
        this.context.run((world, pos) -> {
            ...
            if (!player.isInCreativeMode() && blockState.isIn(BlockTags.ANVIL) && player.getRandom().nextFloat() < 0.32F) {
                BlockState blockState2 = GoldAnvilBlock.getLandingState(blockState);
                if (blockState2 == null) {
    ...
    public void updateResult() {
        ...
        if (!itemStack.isEmpty() && EnchantmentHelper.canHaveEnchantments(itemStack)) {
            ...
            l += (long) itemStack.getOrDefault(DataComponentTypes.REPAIR_COST, 0) + itemStack3.getOrDefault(DataComponentTypes.REPAIR_COST, 0);
                ...
                if (itemStack2.isDamageable() && itemStack2.getItem().canRepair(itemStack, itemStack3)) {
                    ...
                    for (Entry<RegistryEntry<Enchantment>> entry : itemEnchantmentsComponent.getEnchantmentEntries()) {
                        ...
                        for (RegistryEntry<Enchantment> registryEntry2 : builder.getEnchantments()) {
                            if (!registryEntry2.equals(registryEntry) && !canBeCombined(registryEntry, registryEntry2)) {
                                bl4 = false;
                                i++;
                            }
                        }
                        ...
                        if (!bl4) {
                            ...
                            if (registryEntry.matches(e -> e == Enchantments.LOYALTY) ||
                                    registryEntry.matches(e -> e == Enchantments.PIERCING)) {
                                if (r > 118) {
                                    r = 118;
                                }
                            } else if (registryEntry.matches(e -> e == Enchantments.LURE) ||
                                    registryEntry.matches(e -> e == Enchantments.QUICK_CHARGE)) {
                                if (r > 5) {
                                    r = 5;
                                }
                            } else if (registryEntry.isIn(EnchantmentTags.CURSE) || registryEntry.matches(e -> e == Enchantments.SILK_TOUCH)) {
                                if (r > 1) {
                                    r = 1;
                                }
                            }
    ...
    private boolean canBeCombined(RegistryEntry<Enchantment> first, RegistryEntry<Enchantment> second) {
        if ((first.getKey().get() == Enchantments.FORTUNE && second.getKey().get() == Enchantments.SILK_TOUCH) ||
                (second.getKey().get() == Enchantments.FORTUNE && first.getKey().get() == Enchantments.SILK_TOUCH))
            return false;
        else if ((first.getKey().get() == Enchantments.DEPTH_STRIDER && second.getKey().get() == Enchantments.FROST_WALKER) ||
                (second.getKey().get() == Enchantments.FROST_WALKER && first.getKey().get() == Enchantments.DEPTH_STRIDER))
            return false;
        else if ((first.getKey().get() == Enchantments.RIPTIDE && second.getKey().get() == Enchantments.LOYALTY) ||
                (second.getKey().get() == Enchantments.LOYALTY && first.getKey().get() == Enchantments.RIPTIDE))
            return false;
        else if ((first.getKey().get() == Enchantments.RIPTIDE && second.getKey().get() == Enchantments.CHANNELING) ||
                (second.getKey().get() == Enchantments.CHANNELING && first.getKey().get() == Enchantments.RIPTIDE))
            return false;
        return true;
    }
