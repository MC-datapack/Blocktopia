...
public class AbstractBirdEntity extends AnimalEntity {
    ...
    protected static class FlyRandomlyGoal extends Goal {
        ...
        public void start() {
            ...
            double d = this.bird.getX() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double e = this.bird.getY() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            double f = this.bird.getZ() + (random.nextFloat() * 2.0F - 1.0F) * 16.0F;
            this.bird.getMoveControl().moveTo(d, e, f, 0.55);
        }
    }
}
